...
import { useCatalog } from '../hooks/useCatalog'; // Import Hook API

export default function Catalog() {
  const { games, taxonomies, loading, fetchCatalog, fetchTaxonomies } = useCatalog();
  ...
  // Saat halaman dibuka, langsung tembak API Katalog dan Taxonomies
  useEffect(() => {
    fetchCatalog();
    fetchTaxonomies();
  }, [fetchCatalog, fetchTaxonomies]);

  // Logika Filter (Cocokkan data API dengan Pilihan Dropdown)
  ...
  // Mencegah error jika data dropdown belum beres dimuat API
  const cats = taxonomies?.categories || [];
  const subs = taxonomies?.subjects || [];
  const grades = taxonomies?.gradeLevels || [];
    ...
    <div className="p-10 md:p-14 w-full max-w-[1200px] mx-auto">
      <div className="mb-10">
        ...
        <p className="font-['Nunito_Sans'] text-lg text-[#263143] max-w-2xl">
          Temukan ribuan game edukasi buatan rekan guru di seluruh Indonesia.
        </p>
      </div>
          ...
          <div className="flex flex-wrap items-center gap-6">
            <div className="flex items-center gap-3">
              ...
              <div className="relative">
                <select value={activeSubject} onChange={(e) => setActiveSubject(e.target.value)} className="appearance-none cursor-pointer bg-[#DEE8FF] rounded-[18px] py-2 pl-5 pr-10 font-['Quicksand'] font-bold text-[16px] text-[#263143] outline-none focus:ring-2 focus:ring-[#630ED4]">
                  <option value="">Semua</option>
                  {subs.map(subject => <option key={subject} value={subject}>{subject}</option>)}
                </select>
            ...
            <div className="flex items-center gap-3">
              ...
              <div className="relative">
                <select value={activeCategory} onChange={(e) => setActiveCategory(e.target.value)} className="appearance-none cursor-pointer bg-[#DEE8FF] rounded-[18px] py-2 pl-5 pr-10 font-['Quicksand'] font-bold text-[16px] text-[#263143] outline-none focus:ring-2 focus:ring-[#630ED4]">
                  <option value="">Semua</option>
      ...
      {loading ? (
        <div className="text-center py-20 font-['Nunito_Sans'] font-bold text-xl text-[#630ED4] animate-pulse">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-10 lg:gap-12">
          {filteredData.map((game) => (
            <ContentCard key={game.id} game={game} formatPrice={formatPrice} />
          ))}
        </div>
      )}
    </div>
